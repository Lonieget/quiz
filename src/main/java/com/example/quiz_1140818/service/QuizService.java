...
@Service // 要記得@Service不然@Autowired會失敗
public class QuizService {

	// 讓Json跟物件(Object)之間互轉的方法，要寫在try catch中
	private ObjectMapper mapper = new ObjectMapper();

	// import library: org.slf4j.Logger
	...
	// 因為只有要確認有沒有收到前端送來的那包資料，不需要再從後端丟回去收到的資料
	/**
	 * @throws Exception
	 * @Transactional: 1. 使用於修改資料時(insert/update/delete)且又以下2種情形<br>
	 *                 1.1 同一個方法中有使用多個Dao時，例如下面方法中有同時使用 quizDao 和 questionDao<br>
	...
	public BasicRes create(Quiz quiz, List<QuestionVo> questionVoList) throws Exception {
		try {
			...
			if (checkRes != null) { // checkRes != null 表示檢查的結果有錯
				return checkRes;
			}
			...
			for (QuestionVo vo : questionVoList) {
				...
				// 新增問題(question)
				questionDao.create(quizId, vo.getQuestionId(), vo.getName(), optionsStr, //
						vo.getType(), vo.isRequired());

				// 把 vo 中的資料夾以及 optionsStr 一一寫入到 Question 這個 entity 中
				Question entity = new Question(vo.getQuestionId(), vo.getName(), optionsStr, vo.getType(),
						vo.isRequired());
	...
	private BasicRes checkQuestion(List<QuestionVo> questionVoList) {
		for (QuestionVo item : questionVoList) {
			...
			// 檢查選項 使用排除法:D
			// 正確的應該是：type 是1. 簡答題時不會有選項：或 2. 選擇題(單、多選)時要有選項
			// 以下是排除：當 type 是簡答題時不會有選項
			if (!QuestionType.checkChoiceType(item.getType())) {
				if (!item.getOptionsList().isEmpty()) {
					return new BasicRes(ResCodeMessage.QUESTION_TYPE_OPTIONS_MISSMATCH.getCode(), //
							ResCodeMessage.QUESTION_TYPE_OPTIONS_MISSMATCH.getMessage());
				}
...
//			int count = quizDao.selectCountId(quizId); 這句原本存在，不過因為變數只使用一次，所以可以改成128行那樣
			// 原本if(count == 0)長這樣
	...
	public QuizListRes getQuizList(boolean getPublish) {
		if (getPublish) {// isPublish 等同於 Publish == true
	...
	public QuizListRes getQuizList(String title, LocalDate startDate, //
			LocalDate endDate, boolean getPublish) {
		...
		// 到時SQL 中搭配 like %空字串的title% 就把所有 title 的資料撈出來
