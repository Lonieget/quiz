...
import com.example.quiz_1140818.vo.QuestionsCountVo;
...
public class FillinService {
	...
	private BasicRes checkRequiredAnswer(int questionId, List<Answer> answerList, String type) {
		...
		// 如果 answerList 中沒有找到對應的 questionId，則不報錯 (可能在其他地方處理)
		return null;
	}
	...
	public FeedbackRes feedback(int quizId) throws Exception {
		...
		for (Fillin item : fillinList) {
			...
			if (!emailFeedbackVoMap.containsKey(email)) { // 表示尚未記錄到該 user 的答案
				...
				feedbackVo.setQuestionAnswerVoList(questionAnswerVoList);
				emailFeedbackVoMap.put(email, feedbackVo);
			} else {
				feedbackVo = emailFeedbackVoMap.get(email);
				questionAnswerVoList = feedbackVo.getQuestionAnswerVoList();
			}
	...
	public StatisticRes statistic(int quizId) throws Exception {
		...
		Map<Integer, QuestionsCountVo> voMap = setQuestionAnswerVo(quizId);
		// 使用 QuizId 撈取所有的填答
		List<Fillin> fillinList = fillinDao.getByQuizId(quizId);
		// 問題編號, 選項編號 選項 次數
		...
		List<QuestionsCountVo> voList = setAndGetQuestionCountVoList(map, voMap);
		Quiz quiz = quizDao.getById(quizId);
	...
	private Map<Integer, QuestionsCountVo> setQuestionAnswerVo(int quizId) {
		// 將問題相關的資訊設定給 QuestionAnswerVo
		List<Question> questionList = questionDao.getByQuizId(quizId);
		// 將 Question 轉成 QuestionAnswerVo
		Map<Integer, QuestionsCountVo> map = new HashMap<>();
		for (Question question : questionList) {
			// 跳過簡答題
			if (QuestionType.checkTextType(question.getType())) {
				continue;
			}
			QuestionsCountVo vo = new QuestionsCountVo(//
					question.getQuestionId(), question.getName(), //
					question.getType(), question.isRequired());
			// 問題編號，vo
			map.put(question.getQuestionId(), vo);
	...
	private List<QuestionsCountVo> setAndGetQuestionCountVoList(Map<Integer, Map<Integer, Map<String, Integer>>> map, //
			Map<Integer, QuestionsCountVo> voMap) {
		List<QuestionsCountVo> voList = new ArrayList<>();
...
//			});
			...
			QuestionsCountVo vo = voMap.get(questionId);
