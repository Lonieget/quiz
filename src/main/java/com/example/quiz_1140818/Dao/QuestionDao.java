...
public interface QuestionDao extends JpaRepository<Question, QuestionId> {
	...
	@Modifying
	@Transactional
	@Query(value = "insert into question (quiz_id, question_id, name, "//
			+ " options_str, type, is_required) values (?1, ?2, ?3, ?4, ?5, ?6)"//
			, nativeQuery = true)
	public void create(int quizId, int questionId, String name, String optionsStr, //
	...
	public List<Question> getByQuizId(int quizId); //因為是用星號，所以可以撈出多筆資料
