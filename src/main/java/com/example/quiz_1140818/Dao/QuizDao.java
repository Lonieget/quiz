...
public interface QuizDao extends JpaRepository<Quiz, Integer>{
	...
	// 找尋欄位 id 當前的最大值，因為 id 是流水號，最大值表示最新新增的一筆資料
		@Query(value = "select max(id) from quiz", nativeQuery = true)
		public int selectMaxId(); 
		
		//因為撈回來的資料沒有要特別幹嘛，所以使用count就好了，只是要比對次數(存不存在)這樣
		// select count(id) 是去搜尋欄位 id 出現的次數，因為 id 是 PK ，所以結果只會是 0 或 1 
		...
		@Query(value = "delete from quiz where "//
				+ " (is_pblish is false or is_publish is true and ?2 > end_date)"//
